# Write your #display_rainbow method here
def display_rainbow(colors=['red', 'orange', 'yellow', 'green', 'blue', 'indigo', 'violet'])
color.sort
  puts "The first letter in the alphabet is: #{colors[0][0].to uppercase}"
  puts "The first letter in the alphabet is: #{colors[1][0].to uppercase}"
  puts "The first letter in the alphabet is: #{colors[2][0].to uppercase}"
  puts "The first letter in the alphabet is: #{colors[3][0].to uppercase}"
  puts "The first letter in the alphabet is: #{colors[4][0].to uppercase}"
  puts "The first letter in the alphabet is: #{colors[5][0].to uppercase}"
  puts "The first letter in the alphabet is: #{colors[6][0].to uppercase}"
end
